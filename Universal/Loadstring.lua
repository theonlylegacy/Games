...
----------------------------------------------------------------------]]
	...
	LuaK = function(a) luaY = a return luaK end
end
...
local FiOne = nil do
	...
	FiOne = function(BCode, Env)
		return wrap_lua_func(stm_lua_bytecode(BCode), Env or {})
	end
end

local VirtualEnvironment = nil do
...
local loadstring = function(String, Environment)
	local Function, Writer, Buffer, Name
	Environment = Environment or VirtualEnvironment() or {}
	Name = Environment.script and Environment.script:GetFullName() or "@input"

	local Success, Error = pcall(function()
		local ZIO = LuaZ:init(LuaZ:make_getS(String), nil)

		if not ZIO then 
			warn("Error: ZIO expected got nil") 
		end

		local Parsed = LuaY:parser(LuaState, ZIO, nil, Name)
		Writer, Buffer = LuaU:make_setS()
		LuaU:dump(LuaState, Parsed, Writer, Buffer)
		Function = FiOne(Buffer.data, Environment)
	end)
